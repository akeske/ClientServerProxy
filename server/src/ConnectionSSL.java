...
public class ConnectionSSL implements Runnable {
	...
	public ConnectionSSL( SSLSocket sslSocket ){
		this.sslSocket = sslSocket;
		try{
			state = new Authorised();

			inFromClient = new BufferedReader(new InputStreamReader(this.sslSocket.getInputStream()));
			outToClient = new DataOutputStream(this.sslSocket.getOutputStream());
			inFromClientString = new BufferedReader(new InputStreamReader (sslSocket.getInputStream(), "UTF-8"));
			outToClientString = new PrintWriter(new OutputStreamWriter(sslSocket.getOutputStream(), "UTF-8"));
		}catch(Exception e){
	...
	public void run() {
		while(!sslSocket.isClosed()){
			try{
				...
				Report.lgr.log(Level.INFO, userName + " connected with IP: " + sslSocket.getInetAddress(), "");
				state.update();
			}catch(Exception e){
				sslSocket = null;
	...
	private class Disconnect implements ServerState{
		...
		public void update(){
			try {
				InetAddress temp = sslSocket.getInetAddress();
				//    Thread.sleep(2000);
				sslSocket.close();
				inFromClient.close();
				outToClient.close();
